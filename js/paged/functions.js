...
function snapElementsToLineHeight(node) {
  ...
  if (node.src && niceHeightRaw > 0) {
    ...
    linesHigh = Math.ceil((niceHeightRaw + lineHeightPx * 0.5) / lineHeightPx);
  } else {
...
function hideWikipediaEditLinks(content) {
  console.log(content);
  content.querySelectorAll(".en\\.wikipedia\\.org span").forEach(function (x) {
    let re = /\[edit\d*\]/;
  ...
  textNodes.forEach((thisTextNode, i) => {
    ...
    if (theText.length > 0) {
      ...
      while (thisTextNode.nextSibling && guard < 10) {
        guard++;
        console.log({ me: thisTextNode, next: thisTextNode.nextSibling });
        if (
          thisTextNode.nextSibling &&
          ["I", "B", "EM", "STRONG"].includes(thisTextNode.nextSibling.tagName)
        ) {
          ...
          theText += " " + thisTextNode.nextSibling.textContent.trim();
          parent.removeChild(thisTextNode.nextSibling);
        } else if (
          thisTextNode.nextSibling &&
...
function imageAltTextForMissingImages(content) {
  content.querySelectorAll("img").forEach((img) => {
    if (img.classList.contains("toc_meta_publisher_icon")) {
      ...
      img.addEventListener("error", (img) => (img.src = "../pond_icon.png"));
    } else {
      img.alt +=
        (img.alt ? " | " : "") +
